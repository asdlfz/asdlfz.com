...
{
  function bl_tabs(){
    $widget_ops = array('classname' => 'bl_tabs', 'description' => '以选项卡的方式显示最近文章，最近评论和标签' );
    $this->WP_Widget('bl_tabs', 'Bluth选项卡', $widget_ops);
  }
  ...
  function form($instance){
	...
	<p>
		<label for="<?php echo $this->get_field_id('show_recent_posts'); ?>">显示最近文章:</label>
		<select id="<?php echo $this->get_field_id('show_recent_posts'); ?>" name="<?php echo $this->get_field_name('show_recent_posts'); ?>">
			<option value="Yes" <?php echo ($show_recent_posts == 'Yes') ? 'selected' : ''; ?>>是</option>
			<option value="No" <?php echo ($show_recent_posts == 'No') ? 'selected' : ''; ?>>否</option>
		</select>
	</p>	
	<p>
		<label for="<?php echo $this->get_field_id('num_posts'); ?>">显示文章数量:</label>
	...
	<p>
		<label for="<?php echo $this->get_field_id('cat_posts'); ?>">显示下面的分类中的文章:</label>
		...
		<p style="font-size:12px;color:#888">按住Ctrl或Command键选择多项.</p>
	</p>
	<p>
		<label for="<?php echo $this->get_field_id('show_recent_comments'); ?>">显示最近评论:</label>
		<select id="<?php echo $this->get_field_id('show_recent_comments'); ?>" name="<?php echo $this->get_field_name('show_recent_comments'); ?>">
			<option value="Yes" <?php echo ($show_recent_comments == 'Yes') ? 'selected' : ''; ?>>是</option>
			<option value="No" <?php echo ($show_recent_comments == 'No') ? 'selected' : ''; ?>>否</option>
		</select>
	</p>
	<p>
		<label for="<?php echo $this->get_field_id('num_comments'); ?>">显示评论数量:</label>
	...
	<p>
		<label for="<?php echo $this->get_field_id('show_tags'); ?>">显示标签:</label>
		<select id="<?php echo $this->get_field_id('show_tags'); ?>" name="<?php echo $this->get_field_name('show_tags'); ?>" value="<?php echo $show_tags; ?>">
			<option value="Yes" <?php echo ($show_tags == 'Yes') ? 'selected' : ''; ?>>是</option>
			<option value="No" <?php echo ($show_tags == 'No') ? 'selected' : ''; ?>>否</option>
		</select>
	</p>
	<p>
		<label for="<?php echo $this->get_field_id('num_tags'); ?>">显示标签数量:</label>
