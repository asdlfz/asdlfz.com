...
class bl_author extends WP_Widget {
	...
	function bl_author(){
		$widget_ops = array('classname' => 'bl_author', 'description' => '设置显示您的信息' );
		$this->WP_Widget('bl_author', 'Bluth作者挂件', $widget_ops);
	}
	...
	?>
	    ...
	    <p>
      		<label for="<?php echo $this->get_field_id('image_bg_uri'); ?>">背景图片:</label><br />
	    ...
	    <p>
      		<label for="<?php echo $this->get_field_id('image_author_uri'); ?>">您的头像: <small>推荐 1:1 ( 90x90 )</small></label><br />
      	...
      	<hr style="background:#ddd;height: 1px;margin: 15px 0px;border:none;">
	    <p>
      		<label for="<?php echo $this->get_field_id('author_name'); ?>">您的名字:</label><br />
	    ...
	    <p>
      		<label for="<?php echo $this->get_field_id('text'); ?>">描述介绍:</label><br />
...
if(!function_exists('hrw_enqueue')){
	...
	{
	  wp_enqueue_media();
	  wp_enqueue_script('hrw', '/wp-content/themes/bliss/assets/js/admin-script.js', array('jquery'), null, true);
	}
