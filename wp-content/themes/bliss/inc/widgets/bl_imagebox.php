...
class bl_imagebox extends WP_Widget {
	...
	function bl_imagebox(){
		$widget_ops = array('classname' => 'bl_imagebox', 'description' => '图片信息展示' );
		$this->WP_Widget('bl_imagebox', 'Bluth图片挂件', $widget_ops);
	}
	...
	?>
	    ...
	    <p>
      		<label for="<?php echo $this->get_field_id('image_bg_uri'); ?>">上传图片:</label><br />
			...
			<input class="button" onClick="bl_open_uploader(this, 'uploaded_imagebox_bg_image')" id="bluth_image_upload" value="上传" />
	    </p>
	    ...
	    <p>
      		<label for="<?php echo $this->get_field_id('imagebox_name'); ?>">图片标题:</label><br />
	    ...
	    <p>
      		<label for="<?php echo $this->get_field_id('footer'); ?>">底部文本:</label><br />
