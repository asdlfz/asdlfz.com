...
class bl_flickr2_widget extends WP_Widget {
	function bl_flickr2_widget() {
		$widget_ops = array( 'classname' => 'bl_flickr', 'description' => __( '国内被封', 'bluth' ) );
	...
	function widget( $args, $instance ) {
		...
		echo !empty($instance['title']) ? '<h3 class="widget-head">'.$instance['title'].'<a href="http://www.flickr.com/photos/' . $username . '" target="_blank">' . $username . '</a></h3>' : '' ?>
  		<div class="widget-body"> 
  			<div class="flickr-images-container">
  				<ul class="flickr-images clearfix flickr-slider">
					...
					if ( $rsp_obj['stat'] == 'ok' ) {
						$isfirst = true;
						foreach ( $rsp_obj['photos']['photo'] as $photo ) { if($isfirst){ $isfirst = false; echo '<li><img src="http://farm' . $photo['farm'] .'.staticflickr.com/' . $photo['server'] . '/' . $photo['id'] . '_' . $photo['secret'] . '.jpg" alt="fail"></li>';  }else{ echo '<li><img style="display:none;" src="http://farm' . $photo['farm'] .'.staticflickr.com/' . $photo['server'] . '/' . $photo['id'] . '_' . $photo['secret'] . '.jpg" alt="fail"></li>'; }}} 
