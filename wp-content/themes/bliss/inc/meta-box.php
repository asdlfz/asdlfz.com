<?php
	...
	{  
	    add_meta_box( 'bluth_post_meta', '页面布局', 'bluth_post_meta', 'post', 'normal', 'high' );
	    add_meta_box( 'bluth_page_side_meta', '页面选项', 'bluth_page_side_meta', 'page', 'normal', 'high' );
	}  
	...
	{  
	    ...
	    <fieldset class="clearfix bl_status_show"> 
		    <div class="cd-layout" style="width:45%;"> 
		        <label for="facebook-status"> 
		            <h4 style="border-bottom:1px solid #cccccc; margin:25px 0; padding-bottom:10px; width:95%;">Facebook embedded status ( <a href="https://developers.facebook.com/docs/plugins/embedded-posts/" target="_blank">我该如何做?</a> )</h4>
		        </label> 
		    ...
		    <div class="cd-layout" style="width:45%;"> 
		        <label for="twitter-status"> 
		            <h4 style="border-bottom:1px solid #cccccc; margin:25px 0; padding-bottom:10px; width:95%;">Twitter embedded status ( <a href="https://dev.twitter.com/docs/embedded-tweets" target="_blank">我该如何做?</a> )</h4>
		        </label> 
		    	<textarea id="twitter-status" name="bluth_twitter_status" value="" style="width:95%"><?php echo $twitter_status; ?></textarea>
		    </div>
		</fieldset> 
	    <fieldset class="clearfix"> 
	    <h4 style="border-bottom:1px solid #cccccc; margin:25px 0;">边栏位置</h4>
	    <div class="cd-layout"> 
	    ...
	    <h4 style="border-bottom:1px solid #cccccc; margin:25px 0;">边栏类型</h4>
	    <fieldset> 
	    	<label for="bluth_post_right_sidebar">右边栏</label>
	    	<select name="bluth_post_right_sidebar" id="bluth_post_right_sidebar" value="<?php echo $right_sidebar; ?>">
				<option value="sidebar_right" <?php selected( $right_sidebar, 'sidebar_right' ); ?>>右边栏</option>
				<option value="sidebar_left" <?php selected( $right_sidebar, 'sidebar_left' ); ?>>左边栏</option>
	    	</select><br><br>
	    	<label for="bluth_post_left_sidebar">左边栏</label>
	    	<select name="bluth_post_left_sidebar" id="bluth_post_left_sidebar" value="<?php echo $left_sidebar; ?>">
				<option value="sidebar_left" <?php selected( $left_sidebar, 'sidebar_left' ); ?>>左边栏</option>
				<option value="sidebar_right" <?php selected( $left_sidebar, 'sidebar_right' ); ?>>右边栏</option>
	    	</select>
	    </fieldset> 
	    <?php 
	} 
	...
	{  
	    ...
	    <p> 
	        ...
	        <label for="bluth_page_hide_title">隐藏标题</label>
	    </p>  
	    <p> 
	        <label for="bluth_page_subtitle">子标题</label>
