...
/* Updated: 18. september 2013 */
	...
	function bluth_tooltip( $atts, $content = null)
	{
		...
		return '<a href="javascript:void(0)" class="tips" data-trigger="' . $trigger . '" data-placement="' . $placement . '" title="' . $text . '">'. do_shortcode($content) . '</a>';
	}
	...
	function bluth_get_posts($atts, $content = null){
		...
		extract( shortcode_atts( array( 'limit' => 5, 'category' => 'uncategorized' ), $atts ) );

		$q = new WP_Query( 'posts_per_page=' . $limit . '&category_name=' . $category );

		$list = '<ul class="bl-recent-posts unstyled">';
		...
		while ( $q->have_posts() ) {
			...
			if ( has_post_thumbnail() ) { 
				...
				$list .= '<a href="' . $src_large[0] . '" class="lightbox pull-left" title="' . get_the_title() . '" rel="bookmark">';
				$list .= '<img src="' . $src_thumb[0] .'">';
				$list .= '</a>';
			}
			$list .='<a href="' . get_permalink() . '"><h4>' . get_the_title() . '</h4></a><small>'  . get_the_date() .  '</small><p>' . get_the_excerpt() . '</p></li>';
		}
	...
	function register_button($buttons) {  
		...
		array_push($buttons, 'alert','label','badge','well','button','blockquote','syntax','tooltip','popover','accordion', 'divider', 'columns', 'dropcap', 'pullquote','progress','tabs', 'extras');  
		return $buttons;  
	} 

	function add_plugin($plugin_array) {  

		$source = plugins_url('bluthcodes') . '/tinymce/tinymce.js';
	   ...
	   $plugin_array['dropcap'] 	= $source;
	   $plugin_array['pullquote'] 	= $source;
	   $plugin_array['progress'] 	= $source;
	   $plugin_array['tabs'] 		= $source;
	   $plugin_array['extras'] 		= $source;
